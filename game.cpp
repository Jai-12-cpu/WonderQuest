...
class Game
{
    ...
    public:
        ...
        void printBanner();
        void printRoomState();
        bool processCommand(const string &rawInput);
};
...
void Game::run()
{
    ...
    while(life)
    {
        ...
        if (!getline(cin, input))
            break;
        bool cont = processCommand(input);
        if (!cont) break;
    }
}
...
int game_js_start()
{
    ...
    g_game->printBanner();
    g_game->printRoomState();
    return 0;
}

EMSCRIPTEN_KEEPALIVE
int game_js_step(const char* cmd)
{
    if (!g_game) return 1;
    bool cont = g_game->processCommand(string(cmd));
    if (cont)
    {
        g_game->printRoomState();
        return 0;
