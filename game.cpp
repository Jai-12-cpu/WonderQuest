...
static bool findItemCaseInsensitive(const vector<string> &items, const string &target, string &found) //if  item in room (ROOM only)
{
...
class Location //1 constructor and 5 getter functions
{
    private:
        string name, desc;
...
class Items //1 constructor and 5 getter functions
{
    private:
...
class Characters //1 constructor and 5 getter functions
{
    private:
...
class Actions
{
    public:
        void pick(map<string,Location> &WorldMap, map<string,Items> &WorldItem, string curr_room, Inventory &inventory, bool &hasJamTart, int &current_score, const string &target)
        {
            ...
            {
                if(!inInv) 
                { //once I add to inventory , I can remove from roomitems vector/list
                    auto it_room = find(roomItems.begin(),roomItems.end(),foundRoomItem);
                    inventory.add(foundRoomItem); 
                    roomItems.erase(it_room); 
                    
                    int pts = 0;
                    try { pts = stoi(WorldItem[toLower(foundRoomItem)].GetScore()); } 
                    catch(...) {}
                    current_score += pts;
...
class Game
{
    private:
        ...
        Inventory inventory;
        Control control;
        Actions actions; //create objetc in another class
